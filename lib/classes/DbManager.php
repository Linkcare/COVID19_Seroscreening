...
abstract class DbManager {
    ...
    /**
     * Returns an ErrorInfo object with information about the execution of the last query
     * Note that this function always returns a non-null ErrorInfo object.<br>
    ...
    public function ExecuteBindQuery($query, $arrVariables = null, $limit = null, $offset = null, $log = false) {
        ...
        try {
            ...
            if ($log && $this->generateLogs) {
                $this->logQuery('sql', $query, $arrVariables, microtime(true) - $elapsed);
            }
        } catch (DbException $e) {
    ...
    protected function isReadQuery($query) {
        return preg_match('~^\s*(SELECT|SHOW).*~', $query);
    }
    ...
    final private function logQuery($type, $query = null, &$arrVariables = null, $duration = 0, $errorMessage = null, $errorCode = null) {
        ...
        if ($this->logFunction) {
            ...
            error_log($dateUTC . ' ' . strtoupper($type) . ' ' . $msg);
        }
    }
