...
class DbManagerMySQL extends DbManager {
    ...
    public function ExecuteSQL($query, $arrVariables = null, $limit = null, $offset = null) {
        ...
        if ($limit !== null && $limit > 0) {
            /*
             * In MySQL and MariaDB, the ORDER BY clause derived queries is ignored.
             * For this reason, when a limit and an offset is specified, we cannot just wrap the requested $query into another query like "SELECT *
             * FROM ($query) LIMIT x,y",
             * because the order of $query may not be preserved.
             * Instead, we must create a temporary table with the results and apply the LIMIT on the temporary table
             */
            $tmpSql = 'DROP TABLE IF EXISTS pag_table';
            $this->ExecuteSQL($tmpSql);
            $tmpSql = "CREATE TEMPORARY TABLE pag_table $query";
            $this->ExecuteSQL($tmpSql, $arrVariables);
            $error = $this->getError();
            if ($error->getErrorCode()) {
                return $this->results;
            }
            $arrVariables = null;
            $wrapper = "SELECT * FROM pag_table ";
            $limitConditions = [];
            ...
            $query = $wrapper . implode(' ', $limitConditions);
        }
    ...
    public function columnSetNullable($tableName, $columnName, $nullable) {
        ...
        if (!$rst->Next()) {
            return new ErrorInfo(ErrorInfo::DATABASE_COLUMN_NOT_FOUND);
        }
    ...
    public function createSchema($schema, $failIfExists = true) {
        ...
        $this->ExecuteBindQuery($sql);
        $error = $this->getError();
        if (!$error->getErrorCode() && !empty($schema->tables)) {
        ...
        if (!$error->getErrorCode() && !empty($schema->foreignKeys)) {
    ...
    public function createTable($table) {
        ...
        foreach ($table->columns as $column) {
            ...
            if ($column->defaultValue !== null) {
                $defaultValue = ' DEFAULT ' .
                        (is_string($column->defaultValue) || !is_numeric($column->defaultValue) ? "'" . SQLHelper::escapeString($column->defaultValue) .
                        "'" : $column->defaultValue);
            }
        ...
        if (!$error->getErrorCode() && $table->primaryKey && count($table->primaryKey) > 1) {
            $error = $this->createPrimaryKey($table->name, $table->primaryKey);
        }

        if (!$error->getErrorCode() && count($table->indexes) > 0) {
        ...
        if ($error->getErrorCode() && $tableCreated) {
    ...
    public function createColumn($tableName, $colDef) {
        ...
        if ($colDef->defaultValue !== null) {
            $defaultValue = ' DEFAULT ' .
                    (is_string($colDef->defaultValue) || !is_numeric($colDef->defaultValue) ? "'" . SQLHelper::escapeString($colDef->defaultValue) .
                    "'" : $colDef->defaultValue);
        }
