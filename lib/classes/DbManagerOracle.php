...
class DbManagerOracle extends DbManager {
    ...
    public function ExecuteSQL($query, $arrVariables = null, $limit = null, $offset = null) {
        ...
        if (($limit !== null && $limit > 0) || ($offset !== null && $offset > 0)) {
            $wrapper = 'SELECT * FROM (SELECT ROWNUM AS "$_RN", "_sys_wrap".* FROM (' . $query . ') "_sys_wrap") WHERE ';
            $limitConditions = [];
            if ($offset) {
                $limitConditions[] = '"$_RN" >= ' . $offset;
            } else {
                $offset = 1;
            }
            if ($limit) {
                $limitConditions[] = '"$_RN" < ' . ($offset + $limit);
            }

            $query = $wrapper . implode(' AND ', $limitConditions);
        }
    ...
    public function columnSetNullable($tableName, $columnName, $nullable) {
        ...
        if ($rst->Next()) {
            ...
            $error = new ErrorInfo(ErrorInfo::DATABASE_EXECUTION_ERROR);
        }
    ...
    public function createSchema($schema, $failIfExists = true) {
        ...
        if (!$error->getErrorCode() && count($schema->sequences) > 0) {
    ...
    public function createTable($table) {
        ...
        if (!$error->getErrorCode() && !empty($table->primaryKey)) {
